...
let split_list l n =
  let rec split_list_aux l l2 c =
    if c = n then (l2, l)
    else split_list_aux (List.tl l) (List.hd l :: l2) (c + 1)
  in
  let l1, l2 = split_list_aux l [] 0 in
  (List.rev l1, l2)

let shuffle n =
  let fifo = Fifo.of_list [ (21, 1); (0, n) ] in
  let rec shuffle_aux fifo i last before_last =
    if i = 0 then fifo
    else
      let c1 = (fst last + 21) mod 55 in
      let c2 =
        if snd last <= snd before_last then snd before_last - snd last
        else snd before_last - snd last + randmax
      in
      let couple = (c1, c2) in
      let fifo = Fifo.push couple fifo in
      shuffle_aux fifo (i - 1) couple last
  in
  let fifo = shuffle_aux fifo 53 (21, 1) (0, n) in
  let sorted =
    List.sort
      (fun a b -> fst a - fst b)
      (Fifo.to_list fifo)
  in
  let list24, list31 = split_list sorted 24 in
  let f1_init = Fifo.of_list (List.map snd list31) in
  let f2_init = Fifo.of_list (List.map snd list24) in
  let tirage f1 f2 =
    ...
    ((f1, f2), d)
  in
  let rec doXtimes f1 f2 x =
    if x = 0 then (f1, f2)
    else
      let (f1, f2), _ = tirage f1 f2 in
      doXtimes f1 f2 (x - 1)
  in
  let f1, f2 = doXtimes f1_init f2_init 165 in
  let list = List.init 52 (fun x -> x) in
  let rec get_permutation f1 f2 acc l =
    if List.length l = 0 then acc
    else
      let (f1, f2), d = tirage f1 f2 in
      let rec extract n l = match l with
        | [] -> raise Not_found
        | h :: t ->
            if n = 0 then (h, t)
            else
              let e, t = extract (n - 1) t in
              (e, h :: t)
      in
      let number, new_list = extract (reduce d (List.length l)) l in
      get_permutation f1 f2 (number :: acc) new_list
  in
  get_permutation f1 f2 [] list
